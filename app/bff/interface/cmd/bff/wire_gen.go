...
func wireApp(confServer *conf.Server, confData *conf.Data, logger log.Logger, registry *nacos.Registry) (*kratos.App, func(), error) {
	...
	creationUseCase := biz.NewCreationUseCase(creationRepo, articleRepo, columnRepo, talkRepo, recovery, logger)
	...
	messageUseCase := biz.NewMessageUseCase(messageRepo, userRepo, recovery, logger)
	bffService := service.NewBffService(userUseCase, creationUseCase, talkUseCase, articleUseCase, columnUseCase, achievementUseCase, newsUseCase, commentUseCase, messageUseCase, logger)
