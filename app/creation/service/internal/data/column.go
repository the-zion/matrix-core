...
func (r *columnRepo) SetColumnContentIrregular(ctx context.Context, review *biz.TextReview) (*biz.TextReview, error) {
	ar := &ColumnContentReview{
		ColumnId: review.CreationId,
		Title:    review.Title,
		Kind:     review.Kind,
		Uuid:     review.Uuid,
	...
	err := r.data.DB(ctx).Select("ColumnId", "Title", "Kind", "Uuid", "JobId", "Label", "Result", "Section").Create(ar).Error
	if err != nil {
		return nil, errors.Wrapf(err, fmt.Sprintf("fail to add column content review record: review(%v)", review))
	}
	review.Id = int32(ar.ID)
	review.CreateAt = time.Now().Format("2006-01-02")
	return review, nil
...
func (r *columnRepo) GetColumnContentReview(ctx context.Context, page int32, uuid string) ([]*biz.TextReview, error) {
	key := "column_content_irregular_" + uuid
	review, err := r.getColumnContentReviewFromCache(ctx, page, key)
	if err != nil {
		return nil, err
	}
	...
	review, err = r.getColumnContentReviewFromDB(ctx, page, uuid)
	if err != nil {
		return nil, err
	}
...
func (r *columnRepo) getColumnContentReviewFromCache(ctx context.Context, page int32, key string) ([]*biz.TextReview, error) {
	if page < 1 {
		page = 1
	}
	index := int64(page - 1)
...
func (r *columnRepo) getColumnContentReviewFromDB(ctx context.Context, page int32, uuid string) ([]*biz.TextReview, error) {
	if page < 1 {
		page = 1
	}
	index := int(page - 1)
