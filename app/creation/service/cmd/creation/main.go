...
func init() {
	...
	flag.StringVar(&nacosConfig, "config", "matrix.creation.service", "nacos config name, eg: -config xxx")
}
...
func main() {
	...
	dataID := nacosConfig
	group := nacosGroup
	_, err = client.PublishConfig(vo.ConfigParam{DataId: dataID, Group: group})
	if err != nil {
		panic(err)
	}
